...
import { crearDocumentoPedido, borrarDocumentoPedido } from "./actions";
...
export function DocumentosPedido({ documentos }: { documentos: Documento[] }) {
  ...
  async function onSubir(file: File) {
    ...
    const resultado = await crearDocumentoPedido({ nombre: file.name, storagePath: ruta });
    setSubiendo(false);
  ...
  async function onBorrar(doc: Documento) {
    ...
    setError(null);
    const resultado = await borrarDocumentoPedido(doc.id, doc.storage_path);
    setBorrandoId(null);
    ...
    <div className="mb-5">
      ...
      {documentos.length === 0 ? (
        <p className="text-mute text-sm text-center py-4 bg-surf border border-edge rounded-[10px]">
          Todavía no hay ningún PDF guardado. Súbelo cuando mandes un pedido al proveedor, para
          tener el histórico.
        </p>
      ) : (
        documentos.map((doc) => (
