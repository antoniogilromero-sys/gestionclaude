...
import { crearDocumentoFactura, borrarDocumentoFactura } from "./actions";
...
export function DocumentosFactura({ documentos }: { documentos: Documento[] }) {
  ...
  async function onSubir(file: File) {
    ...
    const resultado = await crearDocumentoFactura({ nombre: file.name, storagePath: ruta });
    setSubiendo(false);
  ...
  async function onBorrar(doc: Documento) {
    ...
    setError(null);
    const resultado = await borrarDocumentoFactura(doc.id, doc.storage_path);
    setBorrandoId(null);
    ...
    <div className="mb-5">
      ...
      {documentos.length === 0 ? (
        <p className="text-mute text-sm text-center py-4 bg-surf border border-edge rounded-[10px]">
          Todavía no hay ninguna factura guardada. Sube aquí el PDF o la foto para tenerlas todas
          juntas.
        </p>
      ) : (
        documentos.map((doc) => (
